...
const Editcontact: React.FC<Card> = ({
  id,
  name,
  phone,
  title,
  ...
  let objs = { id, name, phone, title, email, company };

  initialValues = objs;

  const [addCard] = useEditCardsMutation();
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(isOpen) => setOpen(isOpen)}>
      <Dialog.Trigger asChild>
        <button className="group text-[14px]  text-violet11 rounded flex items-center w-[100%] h-[25px] px-[15px] relative select-none outline-none  leading-4 text-lg hover:bg-violet11 hover:text-white">
          <img src={"edit.svg"} alt="" className="w-3 mr-2" />
          Edit
        </button>
      </Dialog.Trigger>
      <Dialog.Portal>
